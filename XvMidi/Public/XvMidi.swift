...
open class XvMidiNote:Hashable {
    ...
    public static func == (lhs: XvMidiNote, rhs: XvMidiNote) -> Bool {
        
        //if note and channel are the same, then it's the same note
        if (lhs.note == rhs.note && lhs.channel == rhs.channel) {
            return true
        }
        return false
    }
